...
// Нэг дугаар руу олон IP-ээс дайрахаас хамгаална
const phoneRatelimit = new Ratelimit({
  redis,
...
export async function POST(req: NextRequest) {
  ...
  const { success: phoneOk } = await phoneRatelimit.limit(String(phone))
  if (!phoneOk) {
    return NextResponse.json({ error: 'Энэ дугаарт хэт олон оролдлого хийгдлээ. 15 минутын дараа дахин оролдоно уу.' }, { status: 429 })
  }
